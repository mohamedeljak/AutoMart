...
app.post('/api/v1/order', (request, response) => {
  // console.log(req.query.st);
  response.status(200).send({
    "message": "new order added sucessfuly"
  });
});
...
app.get('/api/v1/orders/all', (request, response) => {
  response.status(200).send({
    "message": "orders/all working"
  });
});
app.get('/api/v1/order/', (request, response) => {
  response.status(200).send({
    "message": "order id  working"
  });
}); //////////////////////////////////////////// Orders
...
module.exports = server; // // token encoding and decoding test
// describe('auth : token', () => {
